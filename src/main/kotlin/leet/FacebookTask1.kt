...
class Query (val type: Int, val value: Int)

val N = 5
val bools = TreeSet<Int>()

fun answerQueries(queries: List<Query>, n: Int): List<Int> {
    val results =  ArrayList<Int>(N)
    for (q in queries) {
        when (q.type) {
            1 -> {
                bools.add(q.value)
                println("SETting ${q.value}")
            }
            2 -> {
                val nearestTrue = bools.ceiling(q.value) ?: -1
                results.add(nearestTrue)
            }
        }
    }
    return results
...
fun main(args: Array<String>) {
    val queries = listOf(Query(2, 3), Query(1, 2), Query(2, 1), Query(2, 3), Query(2, 2))
    val result = answerQueries(queries, 5)

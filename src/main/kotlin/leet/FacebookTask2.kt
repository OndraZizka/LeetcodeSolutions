...
You are given an array A containing leet.getN integers. Your task is to find all subarrays whose average sum is greater than the average sum of the remaining array elements.
...
Note that we'll define the average sum of an empty array to be 0, and we'll define the indicies of the array (for the purpose of output) to be 1 through leet.getN. A subarray that contains a single element will have L1 = R1.
Signature
leet.Subarray[] leet.aboveAverageSubarrays(int[] A)
...
Output
A leet.Subarray is an object with two integer fields, left and right, defining the range that a given subarray covers. Return a list of all above-average subarrays sorted as explained above.
